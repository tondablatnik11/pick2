...
def cached_billing_logic(df_pick, df_vekp, df_vepo, df_cats, queue_count_col, df_likp_tmp, df_sdshp_tmp, df_t031_tmp, txt_uncat):
    aus_category_map = {}import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from modules.utils import t
...
def cached_billing_logic(df_pick, df_vekp, df_vepo, df_cats, queue_count_col, df_likp_tmp, df_sdshp_tmp, df_t031_tmp, txt_uncat):
    ...
    # ABSOLUTNÍ ZDROJ PRAVDY PRO VOLLPALETY (Bez závislosti na VEPO)
    del_to_valid_hus = {}
    for d, grp in vekp_filtered.groupby('Clean_Del'):
        v = set()
        for _, r in grp.iterrows():
            hi = str(r['Clean_HU_Int']).strip().lstrip('0')
            he = str(r['Clean_HU_Ext']).strip().lstrip('0')
            if hi and hi not in ['NAN', 'NONE']: v.add(hi)
            if he and he not in ['NAN', 'NONE']: v.add(he)
    ...
    # NOVÁ NEPŘŮSTŘELNÁ LOGIKA (Spoléhá pouze na křížovou shodu HU v systémech)
    def detect_voll(row):
        ...
        for col in pick_hu_cols:
            if col in row.index and pd.notna(row[col]):
                ...
                if val and val not in ['NAN', 'NONE', ''] and val in valid_hus: 
                    return True
        return False
    ...
    def odvod_kategorii(row):
        ...
        # Pokud má zakázka alespoň jednu neprůstřelnou Vollpaletu, je to Vollpalette.
