...
def render_fu(df_pick, queue_count_col):
    ...
    if not df_fu.empty:
        if 'Storage Unit Type' in df_fu.columns:
            ...
            with c1:
                with st.container(border=True): st.metric("📦 Pickováno KLT", f"{klt_count:,}")
            with c2:
                with st.container(border=True): st.metric("🏭 Pickováno Palet", f"{pal_count:,}")
            with c3:
                with st.container(border=True): st.metric("❓ Ostatní / Nezadáno", f"{len(df_fu) - klt_count - pal_count:,}")
            ...
            st.divider()
            col_fu1, col_fu2 = st.columns([1.5, 2])
            with col_fu1:
            ...
            with col_fu2:
                ...
                if 'Month' in df_fu.columns:
                    ...
                    fig = px.bar(trend_df, x='Month', y='Pocet', color='SU_Category', barmode='stack', 
                                 color_discrete_map={'KLT': '#38bdf8', 'Paleta': '#818cf8', 'Ostatní': '#94a3b8', 'Nezadáno': '#cbd5e1'})
