...
def render_dashboard(df_pick, queue_count_col):
        ...
        with c_r1:
            with st.container(border=True):
                st.metric(
                    ...
                    f"{df_pick['Pohyby_Exact'].sum():,.0f} pohybů"
                )
        with c_r2:
            with st.container(border=True):
                st.metric(
                    ...
                    f"{df_pick['Pohyby_Loose_Miss'].sum():,.0f} pohybů",
                    delta_color="inverse"
                )
    ...
    if (df_pick['Queue'].notna().any() and df_pick['Queue'].nunique() > 1):
        ...
        if not df_q_filter.empty:
            ...
            display_q.columns = ["Queue", "Popis", "Počet TO", "Zakázky", "Prům. lokací", "Prům. pohybů na lokaci", "% Přesně", "% Odhad"]

            fmt_q = {c: "{:.1f} %" if "%" in c else "{:.1f}" for c in display_q.columns if c not in ["Queue", "Popis", "Počet TO", "Zakázky"]}
            styled_q = display_q.style.format(fmt_q).set_properties(subset=["Queue", "Prům. pohybů na lokaci"], **{'font-weight': 'bold', 'color': '#1f77b4', 'background-color': 'rgba(31,119,180,0.05)'})
            
            col_qt1, col_qt2 = st.columns([2.5, 1.5])
            with col_qt1:
                st.dataframe(styled_q, use_container_width=True, hide_index=True)
            with col_qt2:
                # Nový interaktivní Plotly graf
                fig = px.bar(
                    q_sum.drop_duplicates('Queue'), x='Queue', y='prum_pohybu_lokace',
                    title='Náročnost (Pohyby na 1 lokaci)', text_auto='.1f',
                    color='prum_pohybu_lokace', color_continuous_scale='Reds'
                )
